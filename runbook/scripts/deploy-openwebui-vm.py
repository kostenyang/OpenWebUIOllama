#!/usr/bin/env python3
# Deploy an Open WebUI + mcpo host VM.
#
# Defaults (4 vCPU / 4 GB RAM / 100 GB disk):
#   Open WebUI is a Python+SQLite app, mcpo is a thin proxy — both
#   are light. Default 4 GB is enough; bump if you load lots of
#   document RAG into Open WebUI.
#
# Override via CLI args if you want a different IP / name / specs:
#   python3 deploy-openwebui-vm.py [--name N] [--ip I] [--cpu C] [--mem-mb M] [--disk-gb D]
...
DEFAULTS = dict(
    name    = 'openwebui-test',   # don't overwrite the live 10.0.0.64 by default
    ip      = '10.0.0.70',
    mask    = '255.255.254.0',
    gw      = '10.0.0.1',
    cpu     = 4,
...
def main():
    ...
    print('Then: bash install-openwebui-offline.sh <bundle_dir>')
    Disconnect(si)
